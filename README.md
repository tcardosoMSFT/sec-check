...
**Sec-Check** is an **agentic security scanning system** that combines deterministic security tools with large language model reasoning to give you a full picture of the safety of a codebase before you run it.

At its core, Sec-Check works by **auto-discovering every security scanning tool installed on your local machine** — Bandit, ShellCheck, Graudit, Trivy, GuardDog, ESLint, Checkov, OWASP Dependency-Check, and more. It then inspects the target codebase, determines which tools are relevant for the languages and file types it finds, and runs those tools automatically. The raw findings are fed into an LLM that performs **semantic analysis** on top of the deterministic output — reasoning about intent, context, and risk to answer questions such as:

- Is this code **safe to run**, or does it contain intentional malicious behavior?
- Are there **reverse shells, backdoors, data-exfiltration routines, or persistence mechanisms** embedded in the code?
- What are the **security vulnerabilities** (injection flaws, weak cryptography, insecure dependencies) and how should they be fixed?
- How can this codebase be **improved** by combining the structured output of static analysis tools with LLM-powered reasoning?

The result is a **structured Markdown report** with severity levels, line numbers, code snippets, and prioritized remediation advice — generated by the best of both worlds: fast, rule-based scanners and context-aware LLM analysis.

> ⚠️ **Warning**: Sec-Check catches common red flags, not sophisticated zero-day attacks. Always use manual review and sandboxing for high-risk code in production or high-security environments.

---

![sec-check](media/sec-check.jpeg)

[View the results of a full scan →](audit-results/scan-results.md)
...
2. [Components](#components)
   - [VS Code Copilot Toolkit](#vs-code-copilot-toolkit)
     - [Custom Agent](#custom-agent)
     - [Security Skills](#security-skills)
     - [Custom Prompts](#custom-prompts)
     - [Security Remediation Planning](#security-remediation-planning)
   - [Standalone CLI Tool (Sec-Check)](#standalone-cli-tool-agentsec)
     - [What Gets Scanned](#what-gets-scanned)
     - [Parallel Scanning Mode](#parallel-scanning-mode)
     - [Reliability Features](#reliability-features)
     - [Progress Tracking](#progress-tracking)
     - [Configuration](#configuration)
3. [Quick Start](#quick-start)
4. [Installing from a Release](#installing-from-a-release)
   - [One-Command Installers](#one-command-installers-recommended)
   - [Option A — VS Code Extension](#option-a--vs-code-extension-interactive)
   - [Option B — CLI Tool](#option-b--cli-tool-automated--ci)
   - [Option C — Copilot Skills & Prompts](#option-c--copilot-skills--prompts-copilot-toolkit)
5. [Adding Custom Scanning Skills](#adding-custom-scanning-skills)
6. [Output](#output)
7. [Repository Structure](#repository-structure)
8. [Setup & Development](#setup--development)
9. [Version Requirements](#version-requirements-summary)
...
Sec-Check provides a **two-layer security scanning pipeline**:

**Layer 1 — Deterministic tools** (fast, rule-based):
Discovers and runs all locally installed security scanners against the appropriate files in your codebase. No configuration required — the agent classifies your files by language and file type, selects the relevant tools, and runs them.

**Layer 2 — LLM semantic analysis** (context-aware):
Takes the raw output from all scanners and applies large language model reasoning to interpret findings in context. The LLM determines whether flagged patterns represent genuine malicious intent, accidental insecurity, or false positives — and explains *why*.

Together these layers answer:
- Is it **safe to run** this code? (reverse shells, backdoors, exfiltration, destructive payloads)
- What **vulnerabilities** exist and how severe are they? (OWASP Top 10, CVEs, misconfigurations)
- Is there **intentional malicious code** — or code that could be weaponized?
- What **fixes** should be applied, in what order, to harden the codebase?
...
- Run **Sec-Check: Scan Workspace for Security Issues**

Or right-click any folder in the Explorer sidebar → **Sec-Check: Scan Folder for Security Issues**.

Results appear in the **Sec-Check** panel in the Activity Bar.
...
Or from the VS Code Extensions sidebar: find **Sec-Check**, click the gear icon, and select **Uninstall**.
...
Sec-Check discovers Copilot CLI agentic skills from two directories:
...
The skill will be **automatically discovered** the next time Sec-Check starts a scan — no code changes needed. The `discover` command in the VS Code extension and the `--verbose` CLI flag will confirm the skill was picked up.
...
For Sec-Check extension tool discovery, install skill folders under your user-level Copilot skills path:
...
### Standalone CLI Tool (Sec-Check)

Sec-Check is a standalone CLI tool built with the GitHub Copilot SDK that automates security scanning programmatically.
...
Sec-Check uses **Copilot CLI built-in tools** (`bash`, `skill`, `view`) to invoke real security scanners and analyze your code. The agent follows a structured workflow:
...
By default, Sec-Check runs all scanners sequentially in a single LLM session. With `--parallel`, it uses a **sub-agent orchestration** pattern that runs multiple scanners concurrently for faster results:
...
Sec-Check provides real-time progress feedback during scans:
...
Sec-Check can be configured via:
